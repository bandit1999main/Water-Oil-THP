        ...
        <div class="main-border-box">
          ...
          <div class="footer-signatures">
            ...
            <div class="footer-box-right">
              ...
              <div>(<span class="editable-field" contenteditable="true">${poName}</span>)</div>
...
export function printPnd3() {
  const waterEmployees = getWaterEmployees();
  
  // Filter for employees with tax > 0
  const taxEmployees = waterEmployees.filter(item => {
    const allowance = item.workDays * (window.waterAllowancePerDay || 30);
    const tax = calculateWaterTax(item.salary, allowance);
    return tax > 0;
  });
  ...
  if (taxEmployees.length === 0) {
    window.showToast('ไม่มีพนักงานที่ถูกหักภาษี ณ ที่จ่าย ในเดือนนี้!', 'warning');
    return;
  }

  // Sort them
  taxEmployees.sort((a, b) => a.name.localeCompare(b.name, 'th'));

  const globalMonthSelect = document.getElementById('globalMonth');
  const globalYearSelect = document.getElementById('globalYear');
  const monthText = globalMonthSelect ? globalMonthSelect.options[globalMonthSelect.selectedIndex].text : '';
  const yearText = globalYearSelect ? globalYearSelect.value : '';
  const month = parseInt(globalMonthSelect ? globalMonthSelect.value : '1');
  const year = parseInt(globalYearSelect ? globalYearSelect.value : '2569');
  ...
  const sigMakerNameVal = document.getElementById('sigMakerName') ? document.getElementById('sigMakerName').value.trim() : '';
  const sigMakerPosVal = document.getElementById('sigMakerPos') ? document.getElementById('sigMakerPos').value.trim() : '';
  ...
  const paymentDate = `${lastDay} ${monthText} ${yearText}`;
  ...
  taxEmployees.forEach((item, index) => {
    const allowance = item.workDays * (window.waterAllowancePerDay || 30);
    const tax = calculateWaterTax(item.salary, allowance);

    totalAllowanceSum += allowance;
    totalTaxSum += tax;
    ...
    const activeBracket = (window.waterTaxBrackets || [
      { minSalary: 0, maxSalary: 25833, rate: 0.00 },
      { minSalary: 25834, maxSalary: 38333, rate: 0.05 },
      { minSalary: 38334, maxSalary: 55000, rate: 0.10 },
      { minSalary: 55001, maxSalary: 75833, rate: 0.15 },
      { minSalary: 75834, maxSalary: 96666, rate: 0.20 },
      { minSalary: 96667, maxSalary: 9999999, rate: 0.25 }
    ]).find(b => item.salary >= b.minSalary && item.salary <= b.maxSalary);
    const taxRate = activeBracket ? (activeBracket.rate * 100).toFixed(0) : "0";

    const splitAllowance = splitBahtSatang(allowance);
    const splitTax = splitBahtSatang(tax);

    tableRowsHtml += `
      <tr>
        <td style="text-align: center;">${index + 1}</td>
        <td style="text-align: center; font-size: 7.5pt; font-family: monospace;">${empTaxId ? empTaxId.replace(/(\d{1})(\d{4})(\d{5})(\d{2})(\d{1})/, '$1-$2-$3-$4-$5') : ''}</td>
        <td style="text-align: left !important; padding-left: 6px !important; line-height: 1.3;">
          <strong>${item.name}</strong><br>
          <span style="font-size: 7pt; color: #555;">ที่อยู่: ${empAddress || '-'}</span>
        </td>
        <td style="text-align: center;">${paymentDate}</td>
        <td style="text-align: center;">ค่าน้ำดื่มประจำเดือน</td>
        <td style="text-align: center;">${taxRate}%</td>
        <td style="text-align: right; padding-right: 4px; border-right: none !important;">${splitAllowance.baht}</td>
        <td style="text-align: center; border-left: none !important;">${splitAllowance.satang}</td>
        <td style="text-align: right; padding-right: 4px; border-right: none !important;">${splitTax.baht}</td>
        <td style="text-align: center; border-left: none !important;">${splitTax.satang}</td>
        <td style="text-align: center;">1</td>
      </tr>
    `;
  });

  const sumAllowance = splitBahtSatang(totalAllowanceSum);
  const sumTax = splitBahtSatang(totalTaxSum);
  ...
  printWindow.document.write(`
    ...
    <head>
      <meta charset="UTF-8">
      <title>ใบแนบ ภ.ง.ด. 3 - ค่าน้ำดื่มประจำเดือน ${monthText} ${yearText}</title>
      <link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@300;400;600;700&display=swap" rel="stylesheet">
      <style>
        * {
          box-sizing: border-box;
        ...
        .editable-field {
          background-color: rgba(251, 80, 18, 0.04);
          border-bottom: 1px dashed var(--post-orange);
          padding: 1px 3px;
          border-radius: 2px;
          outline: none;
          display: inline-block;
        ...
        .editable-field:focus {
          background-color: rgba(251, 80, 18, 0.1);
          border-bottom: 1px solid var(--post-orange);
        }
    ...
    <body>
      <div class="no-print-header no-print">
        <h2>📄 ใบแนบ ภ.ง.ด. 3 (ค่าน้ำดื่มเจ้าหน้าที่)</h2>
        <button onclick="window.print()">🖨️ สั่งพิมพ์ใบแนบ ภ.ง.ด. 3</button>
      </div>

      <div class="page-container">
        <!-- Top bar -->
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.3rem;">
          <div style="font-size: 7.5pt; font-weight: bold; width: 25%;">
            เลขประจำตัวผู้เสียภาษีอากรของผู้มีหน้าที่หักภาษี ณ ที่จ่าย (13 หลัก)<br>
            <div style="margin-top: 4px;">
              ${formatTaxIdBoxes(poTaxId)}
            </div>
          </div>
          <div style="text-align: center; width: 50%;">
            <strong style="font-size: 13pt;">ใบแนบ ภ.ง.ด. 3</strong><br>
            <span style="font-size: 8.2pt; font-weight: bold;">รายละเอียดการหักภาษีเงินได้ ณ ที่จ่าย สำหรับเงินได้ตามมาตรา 40(5)(6)(7)(8) แห่งประมวลรัษฎากร</span>
          </div>
          <div style="font-size: 7.5pt; width: 25%; text-align: right; line-height: 1.4;">
            ใบแนบชุดที่ <span class="editable-field" contenteditable="true" style="border-bottom: 1px dotted #000; padding: 0 10px;">1</span> แผ่นที่ <span class="editable-field" contenteditable="true" style="border-bottom: 1px dotted #000; padding: 0 10px;">1</span><br>
            ของจำนวนแผ่นทั้งหมด <span class="editable-field" contenteditable="true" style="border-bottom: 1px dotted #000; padding: 0 10px;">1</span> แผ่น
          </div>
        </div>

        <div style="display: flex; justify-content: space-between; font-size: 8pt; margin-bottom: 0.4rem; border-top: 1px solid #000; padding-top: 0.2rem;">
          <span><strong>ชื่อผู้มีหน้าที่หักภาษี ณ ที่จ่าย (หน่วยงาน/ปณ.):</strong> <span class="editable-field" contenteditable="true">${poName}</span></span>
          <span><strong>ประจำเดือนภาษี:</strong> ${monthText} พ.ศ. ${yearText}</span>
        </div>

        <table class="pnd-table" style="width: 100%; border-collapse: collapse; margin-bottom: 0.4rem;">
          <thead>
            <tr>
              <th rowspan="2" style="width: 4%">ลำดับ<br>ที่</th>
              <th rowspan="2" style="width: 12%">เลขประจำตัว<br>ผู้เสียภาษีอากร (13 หลัก)</th>
              <th rowspan="2" style="width: 25%">ชื่อและที่อยู่ของผู้มีเงินได้<br>(ระบุ ตรอก/ซอย ถนน ตำบล/แขวง อำเภอ/เขต จังหวัด)</th>
              <th rowspan="2" style="width: 10%">วัน เดือน ปี<br>ที่จ่าย</th>
              <th rowspan="2" style="width: 15%">ประเภทเงินได้พึงประเมินที่จ่าย<br>(เช่น ค่าเช่า ค่าจ้างทำของ)</th>
              <th rowspan="2" style="width: 6%">อัตรา<br>ภาษี (%)</th>
              <th colspan="2" style="width: 12%">จำนวนเงินที่จ่าย</th>
              <th colspan="2" style="width: 12%">จำนวนเงินภาษีที่หักและนำส่งในครั้งนี้</th>
              <th rowspan="2" style="width: 4%">เงื่อน<br>ไข*</th>
            </tr>
            <tr>
              <th style="border-top: 1px solid #000 !important; width: 9%; font-size: 6.5pt;">บาท</th>
              <th style="border-top: 1px solid #000 !important; width: 3%; font-size: 6.5pt;">สต.</th>
              <th style="border-top: 1px solid #000 !important; width: 9%; font-size: 6.5pt;">บาท</th>
              <th style="border-top: 1px solid #000 !important; width: 3%; font-size: 6.5pt;">สต.</th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr style="font-weight: bold; background: #fafafa;">
              <td colspan="6" style="text-align: right; padding-right: 15px;">รวมเงินที่จ่ายและภาษีที่หักนำส่ง (รวมยอดทุกแผ่น)</td>
              <td style="text-align: right; padding-right: 4px; border-right: none !important;">${sumAllowance.baht}</td>
              <td style="text-align: center; border-left: none !important;">${sumAllowance.satang}</td>
              <td style="text-align: right; padding-right: 4px; border-right: none !important;">${sumTax.baht}</td>
              <td style="text-align: center; border-left: none !important;">${sumTax.satang}</td>
              <td></td>
            </tr>
          </tbody>
        </table>
        ...
        <div style="display: flex; justify-content: space-between; font-size: 7.8pt; margin-top: 0.4rem; align-items: flex-end;">
          <div style="width: 45%; line-height: 1.45;">
            * <strong>เงื่อนไขการหักภาษี:</strong> ระบุ 1 สำหรับ หัก ณ ที่จ่ายปกติ | ระบุ 2 สำหรับ ออกภาษีให้ตลอดไป | ระบุ 3 สำหรับ ออกภาษีให้ครั้งเดียว
            <br>
            <strong>หมายเหตุ:</strong> ข้อมูลพนักงานและค่าน้ำดื่มดึงจากสรุปรายการ ณ เดือน<sup>${monthText} พ.ศ. ${yearText}</sup>
          </div>
          
          <div style="width: 50%; border: 1px solid #000; padding: 8px; text-align: center; background-color: #fafafa; border-radius: 4px;">
            <p style="margin: 0 0 0.4rem 0;">ขอรับรองว่ารายละเอียดดังกล่าวข้างต้นถูกต้องและเป็นความจริงทุกประการ</p>
            <br>
            <p style="margin: 0;">ลงชื่อ..........................................................ผู้จ่ายเงิน/ผู้มีหน้าที่หักภาษี</p>
            <p style="margin: 0.3rem 0 0 0;">( <span class="editable-field" contenteditable="true" style="font-weight: bold;">${sigMakerNameVal || poName}</span> )</p>
            <p style="margin: 0.2rem 0 0 0;">ตำแหน่ง <span class="editable-field" contenteditable="true">${sigMakerPosVal || 'เจ้าหน้าที่ผู้จ่ายเงิน'}</span></p>
            <p style="margin: 0.2rem 0 0 0; color: #444;">ยื่นวันที่ <span class="editable-field" contenteditable="true">${thDate}</span></p>
          </div>
        </div>
      </div>
    </body>
