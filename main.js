...
let copyFromPrevMonthBtn = null;
let loadFromRegistryBtn = null;
...
function showConfirm({ title = 'ยืนยัน', message = '', icon = '⚠️', okText = 'ยืนยัน', okClass = 'btn-danger', onConfirm, onCancel }) {
...
async function handleCopyFromPrevMonth() {
  ...
  if (!currMoSelect || !currYrInput) return;

  const currMo = parseInt(currMoSelect.value);
  const currYr = parseInt(currYrInput.value);
  ...
  const prevYrStr = String(prevYr);

  const targetColl = activeMode === 'fuel' 
    ? `employees_${prevYrStr}_${prevMoStr}` 
    : `water_employees_${prevYrStr}_${prevMoStr}`;

  showToast('กำลังดึงข้อมูลจากเดือนก่อนหน้า...', 'info');

  try {
    const prevList = await fetchEmployeesFromCollection(targetColl);
    if (!prevList || prevList.length === 0) {
      showToast(`ไม่พบข้อมูลพนักงานในเดือนก่อนหน้า (${prevMo}/${prevYrStr})`, 'warning');
      return;
    }
    ...
    if (currentList.length > 0) {
      showConfirm({
        title: 'ตรวจพบข้อมูลพนักงานเดิม',
        message: 'พบข้อมูลพนักงานในเดือนนี้แล้ว คุณต้องการ "เขียนทับ (Overwrite)" ข้อมูลเดิมทั้งหมดใช่หรือไม่?',
        icon: '❓',
        okText: 'เขียนทับทั้งหมด',
        okClass: 'btn-danger',
        onConfirm: async () => {
          await processCopy(prevList, true);
        },
        onCancel: () => {
          setTimeout(() => {
            showConfirm({
              title: 'ผสานข้อมูลพนักงาน',
              message: 'คุณต้องการ "ผสานข้อมูล (Merge)" เฉพาะรายชื่อที่ยังไม่มีเข้ามาเพิ่มใช่หรือไม่? (ข้อมูลเดิมจะไม่หาย)',
              icon: '❓',
              okText: 'ผสานรายชื่อ',
              okClass: 'btn-primary',
              onConfirm: async () => {
                await processCopy(prevList, false);
              }
            });
          }, 300);
        }
    ...
    showToast('เกิดข้อผิดพลาดในการดึงข้อมูลเดือนก่อนหน้า', 'error');
  }
}
...
async function processCopy(prevList, isOverwrite) {
  ...
  let resultList = [];
  if (isOverwrite) {
    resultList = cleanedPrevList;
  } else {
    // Merge: retain currentList, add from cleanedPrevList if name does not exist
    const existingNames = new Set(currentList.map(e => e.name.trim()));
    const newItems = cleanedPrevList.filter(e => !existingNames.has(e.name.trim()));
    resultList = [...currentList, ...newItems];
  }
  ...
  if (activeMode === 'fuel') {
    ...
    waterEmployees = resultList;
    await saveWaterEmployees(waterEmployees);
    renderEmployeeTable();
  }

  showToast(`คัดลอกรายชื่อสำเร็จ! จัดเก็บทั้งหมด ${resultList.length} รายชื่อ`, 'success');
}
  ...
  try {
    ...
    if (currentList.length > 0) {
      showConfirm({
        ...
        okClass: 'btn-danger',
        onConfirm: async () => {
          await processLoadRegistry(mappedList, true);
        },
        onCancel: () => {
          setTimeout(() => {
            showConfirm({
              title: 'ผสานข้อมูลทะเบียนพนักงาน',
              message: 'คุณต้องการ "ผสานข้อมูล (Merge)" เฉพาะรายชื่อที่ยังไม่มีเข้ามาเพิ่มใช่หรือไม่? (ข้อมูลเดิมจะไม่หาย)',
              icon: '❓',
              okText: 'ผสานรายชื่อ',
              okClass: 'btn-primary',
              onConfirm: async () => {
                await processLoadRegistry(mappedList, false);
              }
            });
          }, 300);
        }
    ...
    console.error("Failed to load from personnel registry:", error);
    showToast('เกิดข้อผิดพลาดในการดึงข้อมูลทะเบียนบุคลากร', 'error');
  }
}

async function processLoadRegistry(mappedList, isOverwrite) {
  const currentList = activeMode === 'fuel' ? employees : waterEmployees;
  ...
  if (isOverwrite) {
    ...
    const existingNames = new Set(currentList.map(e => e.name.trim()));
    const newItems = mappedList.filter(e => !existingNames.has(e.name.trim()));
    resultList = [...currentList, ...newItems];
  }
